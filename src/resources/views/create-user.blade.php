...
<div id="users-module">
        <create-user :roles="{{isset($roles) ? json_encode($roles) : json_encode('')}}" :units="{{isset($units) ? $units : json_encode('')}}"
        :editable_user="{{isset($editable_user) ? json_encode($editable_user) : json_encode('')}}"></create-user>
</div>
@endsection
